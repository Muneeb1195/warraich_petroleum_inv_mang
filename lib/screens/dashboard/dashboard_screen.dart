...
class DashboardScreen extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            _buildTodaySummary(context, ref, todaySummary, colorScheme),
            const SizedBox(height: 16),
            _buildActiveShiftCard(context, ref, activeShift, colorScheme),
            const SizedBox(height: 16),
            _buildSalesChart(context, weeklySales, colorScheme),
            const SizedBox(height: 16),
            _buildInventoryAlerts(context, allInventory, colorScheme),
          ],
        ),
      ),
    );
  ...
  Widget _buildTodaySummary(BuildContext context, WidgetRef ref, AsyncValue<Map<String, double>> todaySummary, ColorScheme colorScheme) {
              ...
              Row(
                children: [
                  Expanded(
                    child: _StatCard(
                      title: 'Sales',
                      value: 'Rs. ${summary['sales']!.toStringAsFixed(0)}',
  ...
  Widget _buildActiveShiftCard(
    ...
    return Card(
      child: ListTile(
        ...
        title: Text(
          isActive ? 'Active: ${activeShift.value!.type.toUpperCase()} Shift' : 'No Active Shift',
        ),
        subtitle: Text(
        ...
        onTap: isActive
            ? () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => ShiftDetailScreen(shiftId: activeShift.value!.id),
                ),
              )
            : () => Navigator.push(
                context,
